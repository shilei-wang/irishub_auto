...
public class AccountUtils {
	...
	@Keyword
	public static Map addNewKey(String nameNew, String password) {
		JsonObject HttpBody = new JsonObject()
		HttpBody.addProperty("name",nameNew)
		HttpBody.addProperty("password", password)
		String seed = generateSeeds()
		HttpBody.addProperty("seed", seed)
		String jsonString = new Gson().toJson(HttpBody)
		System.out.println(jsonString)
		ResponseObject response = WS.sendRequest(findTestObject('rest/keys/ICS1_post_keys', [ ('HttpBody') : jsonString, ('lcdIP') : GlobalVariable.lcdIP]))
		System.out.println(response.responseBodyContent)
		...
		JsonSlurper slurper = new JsonSlurper()
		Map object = (Map)slurper.parseText(response.responseBodyContent)
		String name = object.get("name")
		String seed_after = object.get("seed")
		WS.verifyEqual(name, nameNew)
		WS.verifyEqual(seed, seed_after)
		return object
	}

	@Keyword
	public static ResponseObject deleteKey(String name, String password) {
		ResponseObject response = WS.sendRequest(findTestObject('rest/keys/ICS1_delete_keys_name', [ ('name') : name, ('password') : password, ('lcdIP') : GlobalVariable.lcdIP]))
		System.out.println(response.responseBodyContent)
		return response
	}

	@Keyword
	public static ResponseObject authAccountInfo(String address) {
	...
	@Keyword
	public static ResponseObject recoverKey(String name, String password, String seed) {
		ResponseObject response = WS.sendRequest(findTestObject('rest/keys/post_key_add_recover', [ ('name') : name, ('password') : password, ('seed') : seed, ('lcdIP') : GlobalVariable.lcdIP]))
		System.out.println(response.responseBodyContent)
		return response
	}

	@Keyword
	public static ResponseObject updateKey(String name, String new_password, String old_password) {
		ResponseObject response = WS.sendRequest(findTestObject('rest/keys/ICS1_put_keys_name', [ ('name') : name, ('new_password') : new_password, ('old_password') : old_password, ('lcdIP') : GlobalVariable.lcdIP]))
		System.out.println(response.responseBodyContent)
		return response
	}
