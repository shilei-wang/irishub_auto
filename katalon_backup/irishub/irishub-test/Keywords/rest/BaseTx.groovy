...
public class BaseTx {
	...
	@Keyword
	public static ResponseObject transferWithParam(String name, String password, String async, String commit, String simulate, String generate_only, String withdrawAddress, String amount){
		//ResponseObject response = WS.sendRequest(findTestObject('rest/governance/ICS22_get_gov_proposals', [('lcdIP') : GlobalVariable.lcdIP]))
		//System.out.println(response.responseBodyContent)
		String[] BaseTxInfo =  baseTxProduce(name, password)
		JsonParser jsonParser = new JsonParser()
		JsonObject object = (JsonObject)jsonParser.parse(BaseTxInfo[0])
		JsonObject HttpBody = new JsonObject()
		HttpBody.add("base_tx",object)
		HttpBody.addProperty("sender",BaseTxInfo[1])
		HttpBody.addProperty("amount",amount)
		String jsonString = new Gson().toJson(HttpBody)
		System.out.println(jsonString)

		RequestObject reqObj = findTestObject('rest/transactions/ICS20_post_bank_accounts_address_transfers', [ ('HttpBody') : jsonString, ('address') : withdrawAddress, ('lcdIP') : GlobalVariable.lcdIP])
		...
		if(generate_only.equals("true")) {
			params.add(new TestObjectProperty("generate-only", ConditionType.EQUALS, generate_only))
		} else if(generate_only.equals("false")) {
			params.add(new TestObjectProperty("generate-only", ConditionType.EQUALS, generate_only))
		}
	...
	public static String transfer(String BaseTx, String senderAddr, String withdrawAddress, String amount, String fee) {
		...
		RequestObject reqObj = findTestObject('rest/transactions/ICS20_post_bank_accounts_address_transfers', [ ('HttpBody') : jsonString, ('address') : withdrawAddress, ('lcdIP') : GlobalVariable.lcdIP])
		List params = new ArrayList<>()
		params.add(new TestObjectProperty("commit", ConditionType.EQUALS, "true"))
		reqObj.setRestParameters(params)
