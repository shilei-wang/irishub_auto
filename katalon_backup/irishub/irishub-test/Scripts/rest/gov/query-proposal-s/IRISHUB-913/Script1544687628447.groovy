...
response = GovUtils.proposalSubmit(v0, password, data.getValue("title",i), data.getValue("description",i), "TxTaxUsage", data.getValue("deposit",i), "0.0000000001", "Burn", "nothing")
...
re = CmdUtils.Parse(response.responseBodyContent).getAsJsonObject().get("value").getAsJsonObject().get("BasicProposal").getAsJsonObject()
...
WS.verifyEqual(re.get("proposal_type").getAsString(), "TxTaxUsage")
