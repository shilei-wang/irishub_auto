...
for (int i = 1; i <= updateBindingData.getRowNumbers(); i++) {
	...
	//验证bind-type
	WS.verifyEqual(re.get("value").getAsJsonObject().get("binding_type").getAsString(), updateBindingData.getValue("bind-type", i))
	
	//验证deposit
	expect_deposit = CmdUtils.plusIris("1100iris", updateBindingData.getValue("deposit", i))
	deposit_array = re.get("value").getAsJsonObject().get("deposit").getAsJsonArray()	
	...
	price_array = re.get("value").getAsJsonObject().get("price").getAsJsonArray()	
	...
	//验证level	
	level = re.get("value").getAsJsonObject().get("level").getAsJsonObject()
