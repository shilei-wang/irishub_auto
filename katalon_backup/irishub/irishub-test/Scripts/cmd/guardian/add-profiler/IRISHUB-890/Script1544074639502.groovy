...
for (int i = 1; i <= u.td.getRowNumbers(); i++) {
	profiler = CmdUtils.getAddressFromName(CmdUtils.createNewAccount(u.v0, "0iris"), "faa")
	cmd = u.command+" --address="+ profiler
	...
	index = GuardianUtils.findItem(array,profiler)
	...
	WS.verifyEqual(item.get("address").getAsString(), profiler)
...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli guardian add-profiler'.concat(GlobalVariable.chainId).concat(GlobalVariable.node).concat(' --from='+v0)
