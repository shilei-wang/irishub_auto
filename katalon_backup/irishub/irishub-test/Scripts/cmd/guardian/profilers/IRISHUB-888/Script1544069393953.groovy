...
WS.verifyEqual(item.get("description").getAsString(), u.td.getValue("description", 1))
WS.verifyEqual(item.get("type").getAsString(), "Genesis")
WS.verifyEqual(item.get("address").getAsString(), u.v0_faa)
...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli guardian profilers'.concat(GlobalVariable.chainId).concat(GlobalVariable.node).concat(GlobalVariable.json)
	}
}
