...
class Utils {
	...
	public Utils(){
		...
		GuardianUtils.addTrustee(v0, user_faa)
		
		command = 'iriscli guardian delete-trustee --chain-id='.concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node).concat(
				' --from=').concat(v0)+" --address="+user_faa
