...
class Utils {
	...
	public Utils(){
		...
		proposal_id = GovUtils.submitProposal(v0, "1000iris")
		GovUtils.voteProposal(v0, proposal_id, td.getValue("v0_option", 1))
		GovUtils.voteProposal(v1, proposal_id, td.getValue("v1_option", 1))
		
		command[1] = 'iriscli gov query-vote'.concat(GlobalVariable.node).concat(GlobalVariable.json)+" --proposal-id="+proposal_id
		command[2] = 'iriscli gov query-votes'.concat(GlobalVariable.node).concat(GlobalVariable.json)+" --proposal-id="+proposal_id
	}
}
