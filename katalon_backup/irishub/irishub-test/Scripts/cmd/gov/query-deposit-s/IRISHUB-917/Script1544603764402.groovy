...
class Utils {
	...
	public Utils(){
		...
		proposal_id = GovUtils.submitProposal(v0, td.getValue("submit_deposit", 1))
		GovUtils.depositProposal(v0, proposal_id, td.getValue("deposit", 1))

		command[1] = 'iriscli gov query-deposit'.concat(GlobalVariable.chainId).concat(GlobalVariable.node).concat(GlobalVariable.json)+
				" --proposal-id="+proposal_id+" --depositor="+CmdUtils.getAddressFromName(v0,"faa")
		command[2] = 'iriscli gov query-deposits'.concat(GlobalVariable.chainId).concat(GlobalVariable.node).concat(GlobalVariable.json)+
