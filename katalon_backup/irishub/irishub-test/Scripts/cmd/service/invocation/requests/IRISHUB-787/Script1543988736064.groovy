...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli service requests'.concat(GlobalVariable.chainId).concat(GlobalVariable.defchainId).concat(GlobalVariable.bindchainId).concat(GlobalVariable.node)+
			" --service-name="+serviceName+" --provider="+provider
	}
}
