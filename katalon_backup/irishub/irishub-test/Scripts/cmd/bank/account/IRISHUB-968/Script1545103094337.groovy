...
address = re.get("value").getAsJsonObject().get("address").getAsString()
WS.verifyEqual(address, u.v0_faa)
...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli bank account'.concat(GlobalVariable.chainId).concat(GlobalVariable.node).concat(GlobalVariable.json).concat(" "+v0_faa)
	}
}
