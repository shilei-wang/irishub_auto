...
public class ServiceUtils {
	...
	public static String defineService(String account){
		...
		String cmd = 'iriscli service define --chain-id='.concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node).concat(
				' --from=').concat(account)
	...
	public static String call(String account, String provider_account, String serviceName){
		...
		String cmd = 'iriscli service call --chain-id='.concat(GlobalVariable.chainId).concat(' --bind-chain-id=').concat(GlobalVariable.chainId).concat(' --def-chain-id=').concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node).concat(
				' --from=').concat(account)+" --service-name="+serviceName+" --provider="+provider
		...
		return StringUtils.getValueFromKey(response.responseBodyContent, "request-id")
	}
	...
	public static respond(String account, String request_id){
		String cmd = 'iriscli service respond'.concat(' --chain-id=').concat(GlobalVariable.chainId).concat(' --request-chain-id=').concat(GlobalVariable.chainId).concat(' --node=').
				concat(GlobalVariable.node).concat(' --from=').concat(account)+" --request-id="+request_id
	...
	public static String fees(String account, String type){
		...
		String cmd = 'iriscli service fees'.concat(' --node=').concat(GlobalVariable.node)+" "+address
		ResponseObject response = CmdUtils.sendRequest('cmd/CmdWithOneArgs',cmd, "wait")
	...
	public static cleanFees(String account){
		String cmd = 'iriscli service refund-fees'.concat(' --chain-id=').concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node)
				.concat(' --from=').concat(account)
		cmd = CmdUtils.addTxFee(cmd, findTestData('base/tx'), 1)
		ResponseObject response = CmdUtils.sendRequest('cmd/CmdWithOneArgs',cmd, "wait")

		cmd = 'iriscli service withdraw-fees'.concat(' --chain-id=').concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node)
				.concat(' --from=').concat(account)
		cmd = CmdUtils.addTxFee(cmd, findTestData('base/tx'), 1)
	...
	public static disableService(String account, String serviceName){
		String cmd = 'iriscli service disable --chain-id='.concat(GlobalVariable.chainId).concat(' --def-chain-id=').concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node).concat(
				' --from=').concat(account)+" --service-name="+serviceName
		cmd = CmdUtils.addTxFee(cmd, findTestData('base/tx'), 1)
	...
	public static updateBinding(String account, String serviceName, String prices, String deposit){
		String cmd = 'iriscli service update-binding --chain-id='.concat(GlobalVariable.chainId).concat(' --def-chain-id=').concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node).concat(
				' --from=').concat(account)+" --service-name="+serviceName+" --prices="+prices+" --deposit="+deposit
		cmd = CmdUtils.addTxFee(cmd, findTestData('base/tx'), 1)
	...
	public static refundDeposit(String account, String serviceName){
		String cmd = 'iriscli service refund-deposit --chain-id='.concat(GlobalVariable.chainId).concat(' --def-chain-id=').concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node).concat(
				' --from=').concat(account)+" --service-name="+serviceName
		cmd = CmdUtils.addTxFee(cmd, findTestData('base/tx'), 1)
	...
	public static String getBindingInfo(String provider, String serviceName){
		...
		String cmd = 'iriscli service binding --def-chain-id='.concat(GlobalVariable.chainId).concat(' --bind-chain-id=').concat(GlobalVariable.chainId).concat(' --node=')
				.concat(GlobalVariable.node).concat(' --service-name=').concat(serviceName).concat(' --provider=').concat(provider_fva)
