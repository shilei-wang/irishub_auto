...
response = ServiceUtils.serviceBindingDepositRefund(BaseTxInfo[0], BaseTxInfo[1], "test", GlobalVariable.chainId)
WS.verifyEqual(StringUtils.stringContains(response.responseBodyContent,refundData.getValue("cmd_result", 1)), true)
AccountUtils.waitUntilNextBlock()

//对非自己绑定的服务执行取回抵押命令。
BaseTxInfo = BaseTx.baseTxProduce(v1, password)
response = ServiceUtils.serviceBindingDepositRefund(BaseTxInfo[0], BaseTxInfo[1], serviceName, GlobalVariable.chainId)
WS.verifyEqual(StringUtils.stringContains(response.responseBodyContent,refundData.getValue("cmd_result", 2)), true)
AccountUtils.waitUntilNextBlock()

//对已绑定，未失效的服务执行取回抵押金额命令。
BaseTxInfo = BaseTx.baseTxProduce(v0, password)
response = ServiceUtils.serviceBindingDepositRefund(BaseTxInfo[0], BaseTxInfo[1], serviceName, GlobalVariable.chainId)
WS.verifyEqual(StringUtils.stringContains(response.responseBodyContent,refundData.getValue("cmd_result", 3)), true)
AccountUtils.waitUntilNextBlock()
...
WS.verifyEqual(StringUtils.stringContains(response.responseBodyContent,"hash"), true)
AccountUtils.waitUntilNextBlock()
//error refund deposit
BaseTxInfo = BaseTx.baseTxProduce(v0, password)
response = ServiceUtils.serviceBindingDepositRefund(BaseTxInfo[0], BaseTxInfo[1], serviceName, GlobalVariable.chainId)
WS.verifyEqual(StringUtils.stringContains(response.responseBodyContent, refundData.getValue("cmd_result", 4)), true)
AccountUtils.waitUntilNextBlock()