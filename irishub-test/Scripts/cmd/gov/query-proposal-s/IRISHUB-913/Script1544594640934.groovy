...
WS.verifyEqual(re.get("proposal_type").getAsString(), u.td.getValue("type", i))
...
class Utils {
	...
	public Utils(){
		...
		command[1] = 'iriscli gov query-proposal '.concat(' --node=').concat(GlobalVariable.node)
		command[2] = 'iriscli gov query-proposals --limit=1'.concat(' --node=').concat(GlobalVariable.node)
	}
}
