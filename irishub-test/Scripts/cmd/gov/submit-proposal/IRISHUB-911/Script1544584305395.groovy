...
for (int i = 1; i <= u.td.getRowNumbers(); i++) {
	...
	status = GovUtils.getProposalStatus(StringUtils.getValueFromKey(response.responseBodyContent, "proposal-id"))
...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli gov submit-proposal --chain-id='.concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node).concat(
				' --from=').concat(v0)
