...
for (int i = 1; i <=u.td.getRowNumbers() ; i++) {	
	proposal_id = GovUtils.submitProposal(u.v0, "Text", u.td.getValue("submit_deposit", i))
...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli gov deposit --chain-id='.concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node).concat(
				' --from=').concat(v0)
