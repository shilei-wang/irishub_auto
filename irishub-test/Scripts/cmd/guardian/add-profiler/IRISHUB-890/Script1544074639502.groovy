...
for (int i = 1; i <= u.td.getRowNumbers(); i++) {
	...
	cmd = u.command+" --profiler-address="+ profiler
	...
	WS.verifyEqual(item.get("name").getAsString(), u.td.getValue("profiler-name", i))
	WS.verifyEqual(item.get("addr").getAsString(), profiler)
...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli guardian add-profiler --chain-id='.concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node).concat(
				' --from=').concat(v0)
