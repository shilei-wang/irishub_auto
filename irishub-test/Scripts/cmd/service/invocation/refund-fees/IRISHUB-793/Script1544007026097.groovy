...
//等待服务响应超时，根据调用请求返回的request id，消费者取回自己的服务费退费。
CmdUtils.waitUntilSeveralBlock(5)
balance_before = CmdUtils.getBalance(u.v0, "name")
expected = Double.valueOf(ServiceUtils.fees(u.v0, "returned_fee").replace("iris", ""))
...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli service refund-fees'.concat(' --chain-id=').concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node)
				.concat(' --from=').concat(v0)
