...
for (int i = 1; i <= u.td.getRowNumbers(); i++) {
	...
	re = CmdUtils.Parse(bindInfo).getAsJsonObject()
	
	//验证bind-type
	WS.verifyEqual(re.get("binding_type").getAsString(), u.td.getValue("bind-type", i))
	
	//验证deposit
	expect_deposit = CmdUtils.plusIris("1000iris", u.td.getValue("deposit", i))
...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli service update-binding --chain-id='.concat(GlobalVariable.chainId).concat(' --def-chain-id=').concat(GlobalVariable.chainId).concat(' --node=').concat(GlobalVariable.node).concat(
				' --from=').concat(v0)+" --service-name="+serviceName
