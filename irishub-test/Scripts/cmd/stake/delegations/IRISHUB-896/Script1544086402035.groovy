...
index = StakeUtils.findItemInDelegations(array,u.v0_fva)
...
index = StakeUtils.findItemInDelegations(array,u.v1_fva)
WS.verifyEqual((index!=-1), true)
item = array.get(index).getAsJsonObject()
WS.verifyEqual(item.get("delegator_addr").getAsString(), u.user_faa)
...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli stake delegations'.concat(' --node=').concat(GlobalVariable.node)+" "+user_faa	
	}
}
