...
class Utils {
	...
	public Utils(){
		...
		command = 'iriscli stake redelegation'.concat(' --node=').concat(GlobalVariable.node).concat(
				' --address-delegator=').concat(user_faa).concat(" --address-validator-source=").concat(v0_fva).concat(" --address-validator-dest=").concat(v1_fva)
	}
}
